...
$abstractxx_name =$FILES['tmp_name'];
     ...
     if(strrchr($abstractxx_name,".")==".pdf" || strrchr($abstractxx_name,".")==".doc" ||strrchr($abstractxx_name,".")==".xls" || strrchr($abstractxx_name,".")==".ppt" || strrchr($abstractxx_name,".")==".docx" || strrchr($abstractxx_name,".")==".xlsx" || strrchr($abstractxx_name,".")==".pptx" || strrchr($abstractxx_name,".")==".zip" || strrchr($abstractxx_name,".")==".rar" || strrchr($abstractxx_name,".")==".mp3"|| strrchr($abstractxx_name,".")==".tar.gz") {	
		 copy($_FILES['abstractxx']['tmp_name'], "data/download_".TIMESTAMP."_".$FILES['name']."");
}else{
		echo "<script language='javascript'>" ;
		echo "alert('"._WEBBOARD_MESS_TPYE_FILE_NOACC." .doc , .xls , .ppt , .pdf , .zip , .tar.gz , .rar , .mp3"._WEBBOARD_MESS_TPYE_FILE_NOACC1."')" ;
		echo "</script>" ;
		echo "<script language='javascript'>javascript:history.back()</script>";
		exit();
	}
