...
$detail=$_POST['detail'];
		...
		$ProcessOutput .= "<A HREF=\"?name=admin&file=video2\"><B>"._ADMIN_VIDEO_MESSAGE_GOBACK." </B></A>";
...
<FORM NAME="myform" METHOD=POST enctype="multipart/form-data" ACTION="?name=admin&file=video2&op=video_edit&action=edit&id=<?php  echo $_GET['id'];?>">
<center><table border=1 bgcolor=#F7F7F7 bordercolor=#FFFFFF width=650 class="grids"><tr><td  align="right" width="100"><?php  echo _ADMIN_VIDEO_FORM_SELECT_CAT;?></td><td>
<SELECT id="CATEGORY" NAME="CATEGORY" onchange="javascript:$('#file_upload').uploadifySettings('scriptData',{'cat':$('#CATGORY').val()});">
<?php 
...
<?php 
	...
	if(CheckLevel($admin_user,$op)){
		while(list($key, $value) = each ($_POST['list'])){
			...
			$res['video'] = $db->select_query("SELECT * FROM web_video2 WHERE id='".$value."' ");
			$arr['video'] = $db->fetch($res['video']);

			$db->del("web_video2"," id='".$value."' "); 
	...
	echo $ProcessOutput ;
	   echo "<meta http-equiv='refresh' content='0;url=?name=admin&file=video2'>" ; 
}
...
if($_GET['id']){
	...
	$limit = 15 ;
	$SUMPAGE = $db->num_rows("web_video2","id"," category=".$_GET['id']."");
	...
	$totalpage = ($rt!=0) ? floor($SUMPAGE/$limit)+1 : floor($SUMPAGE/$limit); 
	$goto = ($page-1)*$limit ;
	$res['cat'] = $db->select_query("SELECT * FROM web_video_category2 WHERE id='".$_GET['id']."' ");
...
$count=0;
$res['video'] = $db->select_query("SELECT * FROM web_video2 WHERE category='".$arr['cat']['id']."' ORDER BY id DESC LIMIT $goto, $limit");
...
?>
	 ...
	 <td width="<?php  echo _IVIDEOT_W+35;?>" colspan="2" >
	 			  						<table cellspacing=0 cellpadding=0 border=0 class='iconframe' ><tr><td  border=0  align="center"><a HREF="index.php?name=video2&file=readvideo&id=<?php  echo $arr['video']['id'];?>" ><img src="<?php if ($arr['video']['pic']){echo "video/thumbs/".$arr['video']['pic'].""; } else { echo "images/video_blank.gif";}?>"></a>
...
<tr>
<td align="left"><a HREF="index.php?name=video2&file=readvideo&id=<?php  echo $arr['video']['id'];?>" ><?php  echo $arr['video']['detail'];?></a>
</td>
</tr>
<tr>
<td align="left">
<a HREF="index.php?name=video2&file=readvideo&id=<?php  echo $arr['video']['id'];?>" ><img src="images/icon-view.gif" border="0"></a> <?php  if($admin_user){?><a href="javascript:Confirm('?name=admin&file=video2&op=video_del&cat=<?php  echo $CAT;?>&id=<?php  echo $arr['video']['id'];?>&pic=<?php  echo $arr['video']['pic'];?>&cats=<?php  echo $arr['cat']['id'];?>&prefix=<?php  echo $arr['video']['post_date'];?>','<?php  echo _ADMIN_BUTTON_DEL_MESSAGE;?>');"><img src="images/admin/trash.gif"  border="0" alt="<?php  echo _ADMIN_BUTTON_DEL;?>" ></a><?php }?>
</td>
</tr>
<tr>
