...
<?php $res['download'] = $db->select_query("SELECT * FROM ".TB_DOWNLOAD." ORDER BY id DESC LIMIT $goto, $limit ");
$count=0;
	...
	echo getfilesize($bytes) ;?> 
	 ...
	 if ($arr['download']['type']=="application/pdf") {
		 ...
		 ?>
		  ...
		  <?php 		 }else if ($arr['download']['type']=="image/gif" || $arr['download']['type']=="image/jpg"|| $arr['download']['type']=="image/jpeg"|| $arr['download']['type']=="image/x-png" ) {
		 ?>
...
<?php 	SplitPage($page,$totalpage,"?name=admin&file=download");
	echo $ShowSumPages ;
	echo "<BR>";
	echo $ShowPages ;
}
