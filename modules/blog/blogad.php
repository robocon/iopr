...
if ($FILE['name']) {
        ...
        $match = preg_match('/(image\/jpe?g)/', $FILE['type']);
		if ($match == 0){
			echo "<script language='javascript'>" ;
...
$abstractxx_name =$FILESS['tmp_name'];
    ...
    $match = preg_match('/(\.pdf|\.doc|\.xls|\.ppt|\.docx|\.xlsx|\.pptx|\.zip|\.rar|\.tar.gz)/', $FILESS['name']);
     if($match > 0) {
//		 copy($_FILES['abstractxx']['tmp_name'], "data/download_".TIMESTAMP."_".$FILES['name']."");
		...
		$ProcessOutput .= "<A HREF=\"?name=blog&file=blogad\"><B>"._BLOG_MOD_BACK."</B></A>";
...
if ($admin_user==$arr['blog']['posted']){
		...
		$ProcessOutput .= "<A HREF=\"?name=blog&file=blogad\"><B>"._BLOG_MOD_BLOG."</B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
